 ...
 */
public class PatientContacts25 extends PreloadedGraphParser{

	public static final String dataPath = "data/PatientContacts25/patients_raw_25_contacts.csv";
	...
	public DyDataSet parse(Mode mode) throws URISyntaxException {
		...
		try {
			InputStream fileStream = PatientContacts25.class.getClassLoader().getResourceAsStream(dataPath); 
			...
			for (int i = 1; i < lines.size(); i++) {
				...
				long startEpoch = Long.parseLong(tokens[2]);
				long endEpoch = Long.parseLong(tokens[3]);

				minEpoch = Math.min(minEpoch, startEpoch);
				maxEpoch = Math.max(maxEpoch, endEpoch);

				LocalDateTime startTime =
					LocalDateTime.ofInstant(Instant.ofEpochMilli(startEpoch), ZoneOffset.UTC);
				LocalDateTime endTime =
					LocalDateTime.ofInstant(Instant.ofEpochMilli(endEpoch), ZoneOffset.UTC);
		...
		double resolution = 100.0 / timeSpan;
		Commons.scatterNodes(graph, 100);
		Commons.mergeAndColor(graph, startTime, endTime, mode, new Color(141, 211, 199), Color.BLACK, 60); // Maybe adjust fadingDuration?
